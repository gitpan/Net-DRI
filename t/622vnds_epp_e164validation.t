...
$dri->target('VNDS')->new_current_profile('p1','Net::DRI::Transport::Dummy',[{f_send=>\&mysend,f_recv=>\&myrecv}],'Net::DRI::Protocol::EPP',['1.0',['E164Validation']]);
...
$cs->set($c2,'admin');
$cs->set($c2,'tech');
$rc=$dri->domain_create_only('5.1.5.1.8.6.2.4.4.1.4.e164.arpa',{duration=>DateTime::Duration->new(years=>1),ns=>$dri->local_object('hosts')->set(['ns1.example.com'],['ns2.example.com']),contact=>$cs,auth=>{pw=>'2fooBAR'},e164_validation_information=>[['EK77','urn:ietf:params:xml:ns:e164valex-1.1',{method_id=>'Validation-X',validation_entity_id=>'VE-NMQ',registrar_id=>'Client-X',execution_date=>DateTime->new(year=>2004,month=>4,day=>8),expiration_date=>DateTime->new(year=>2004,month=>10,day=>7)}]]});
